...
import VideoSection from "@/components/ContentSection/video";
import ExtraContentSection from "@/components/ContentSection/extraContent";
import AnnotationSection from "@/components/ContentSection/annotation";

export default function LessonPage() {
  const [inputValue, setInputValue] = useState<string>("");

  const router = useRouter();
  const { studyTopicId, lessonId } = useParams();
  ...
  useEffect(() => {
    Promise.all([retrieveLesson(lessonId), retrieveStudyTopic(studyTopicId)]);
  }, []);

  // const filteredLessons = lessons.filter((lesson) =>
  //   lesson.title.toLowerCase().includes(inputValue.toLowerCase())
  // );
console.log(selectedStudyTopic?.title)
  return (
    <section className="flex h-full flex-col gap-5 py-6">

      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
        <h3 className="text-4xl font-bold">{selectedStudyTopic?.title}</h3>
          <RiArrowLeftLine
            size={35}
